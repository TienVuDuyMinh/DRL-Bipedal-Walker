...
class BipedalWalker(gym.Env, EzPickle):
    ...
    def __init__(self, render_mode: Optional[str] = None, hardcore: bool = False, mode: str = "default"):
        
        super().__init__(render_mode=render_mode, hardcore=hardcore)
        self.mode = mode  # Lưu chế độ custom: v1, v2, v3
    ...
    def _generate_terrain(self, hardcore):
        ...
        if self.mode == "v1":
            ...
            TERRAIN_LENGTH_V1 = 250
            self.terrain_x = []
            self.terrain_y = []
            y = TERRAIN_HEIGHT
            for i in range(TERRAIN_LENGTH_V1):
                x = i * TERRAIN_STEP
                self.terrain_x.append(x)
                y += self.np_random.uniform(-1.5, 1.5) / SCALE
                self.terrain_y.append(y)
            # — Bắt đầu tạo bodies giống mặc định —
            self.terrain = []
            self.terrain_poly = []
            for i in range(len(self.terrain_x) - 1):
                v1 = (self.terrain_x[i],   self.terrain_y[i])
                v2 = (self.terrain_x[i+1], self.terrain_y[i+1])
                self.fd_edge.shape.vertices = [v1, v2]
                t = self.world.CreateStaticBody(fixtures=self.fd_edge)
                color = (76, 255 if i % 2 == 0 else 204, 76)
                t.color1 = t.color2 = color
                self.terrain.append(t)
                self.terrain_poly.append(([(v1[0], 0), (v2[0], 0), v2, v1], (76,153,76)))
            # Không return ở đây để code bên dưới còn xử lý tiếp


        if self.mode == "v2":
            self.wind_strength = self.np_random.uniform(10, 30)  # Gió từ 10 đến 30 N
            self.wind_dir = 1 if self.np_random.random() < 0.5 else -1
    ...
    def _spawn_falling_rocks(self, step_count):
        if step_count % 80 == 0:  # Mỗi 80 bước lại tạo 1 viên đá
            x = self.hull.position[0] + self.np_random.uniform(-2, 2)
            y = self.hull.position[1] + 5.0
            rock = self.world.CreateDynamicBody(
                position=(x, y),
                fixtures=fixtureDef(
            ...
            rock.color1, rock.color2 = (100, 100, 100), (50, 50, 50)
            self.drawlist.append(rock)
    ...
    def step(self, action: np.ndarray):
        ...
        if self.mode == "v2":
            force = self.wind_strength * self.wind_dir
            self.hull.ApplyForceToCenter((force, 0), wake=True)

        self.step_counter += 1
        
        if self.mode == "v3":
            self._spawn_falling_rocks(self.step_counter)
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if terminated or truncated:
            break
# --- Các lớp custom để toggle mode nhanh ---
class CustomBipedalWalkerV1(BipedalWalker):
    def __init__(self, render_mode=None):
        super().__init__(render_mode=render_mode)
        self.mode = "v1"

class CustomBipedalWalkerV2(BipedalWalker):
    def __init__(self, render_mode=None):
        super().__init__(render_mode=render_mode)
        self.mode = "v2"

class CustomBipedalWalkerV3(BipedalWalker):
    def __init__(self, render_mode=None):
        super().__init__(render_mode=render_mode)
        self.mode = "v3"
